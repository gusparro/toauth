...
import com.gusparro.toauth.domain.entities.AppUser_;
    ...
    public static Specification<AppUser> searchFilter(AppUserSearchFilter searchFilter) {
        return (root, query, builder) -> {
            ...
            if (StringUtils.hasLength(searchFilter.fullName())) {
                predicates.add(builder.like(builder.upper(root.get(AppUser_.fullName)), "%" + searchFilter.fullName().toUpperCase() + "%"));
            }

            if (StringUtils.hasLength(searchFilter.email())) {
                predicates.add(builder.like(builder.upper(root.get(AppUser_.email)), "%" + searchFilter.email().toUpperCase() + "%"));
            }

            if (StringUtils.hasLength(searchFilter.username())) {
                predicates.add(builder.like(builder.upper(root.get(AppUser_.username)), "%" + searchFilter.username().toUpperCase() + "%"));
            }
